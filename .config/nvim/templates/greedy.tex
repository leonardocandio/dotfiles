...
\title{\Huge{ADA}\\Greedy}
\author{\huge{Leonardo Candio}}
\date{}
...
\chapter{}
\section{Problem}
\qs{}{Insert question here (Huffman, knapsack, activity selection, etc)}
\clm{Problem description}{
	\begin{itemize}

	    \item Problem: What is the input?
	    \item Solution: What is a solution
	    \item Optimization: What makes it optimal?
	    \item Greedy choice: What is it that we assume?
	    \item Subproblem: What remains after making the greedy choice?

	\end{itemize}
}
\section{Solution}
\subsection{For the Greedy Choice Property}
\begin{myproof}
	\exists $O$ that is an optimal solution of the given problem that contains $g$. Let $X$ be an arbitrary optimal solution.

	From these assumptions, we derive 2 cases

	

\end{myproof}

\subsection{For the Optimal Substructure Property}
\begin{myproof}
	\exists $O$ that is an optimal solution of the given problem. Let $X$ be an arbitrary optimal solution.
